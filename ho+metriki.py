...
def canberra_distance(x, y):
    if len(x) != len(y):
        raise ValueError("Векторы должны иметь одинаковую длину")
    distance = sum(abs(x[i] - y[i]) / (abs(x[i]) + abs(y[i])) for i in range(len(x)))
    return distance

result = canberra_distance(points2[0], points2[1])
print(f"Расстояние Канберры между векторами {points2[0]} и {points2[1]}: {result}")
result = manhattan_distance(points2[0], points2[1])
print(f"Манхэттенское расстояние между векторами {points2[0]} и {points2[1]}: {result}")
result = euclidean_distance(points2[0], points2[1])
print(f"Евклидово расстояние между векторами {points2[0]} и {points2[1]}: {result}")
p_value = 3
result = minkowski_distance(points2[0], points2[1], p_value)
print(f"Метрика Минковского между векторами {points2[0]} и {points2[1]} с параметром {p_value}: {result}")
